...
<%
	...
	String admin_name = (String) session.getAttribute(ISiteConstant.SESSION_NAME_ADMIN_NAME);
	...
	String compCd 	= (String) session.getAttribute(ISiteConstant.SESSION_NAME_COMPCD);
	//String mkEventList 	= (String) session.getAttribute(ISiteConstant.SESSION_MKEVENT_LIST);
...
<%
	...
	String loginid = (login_type.equals(ISiteConstant.LOGIN_TYPE_ADMIN)) ? admin_name : agency_name + "지국 (" + agency_userid + ")";
	...
	String homeYn	= request.getParameter("homeYn");
	
	//메뉴아이디 세션에 저장
	if(topMenuId != null) { 
		session.setAttribute("topMenuId", request.getParameter("topMenuId")); 
	} else {
		topMenuId = (String)session.getAttribute("topMenuId");
	}
	if(topMenuNm != null) {
		session.setAttribute("topMenuNm", request.getParameter("topMenuNm")); 
	} else {
		topMenuNm = (String)session.getAttribute("topMenuNm");
	}
	if(subMenuId != null) {
		session.setAttribute("subMenuId", request.getParameter("subMenuId")); 
	} else {
		subMenuId = (String)session.getAttribute("subMenuId");
	}
%>
 ...
 * 자동이체독자관리 등록 팝업
 */
function fn_popReaderEdit(opUrl) {
	var eduMisuUrl = "/reader/education/educationUnpaid.do";
	var width="750";
	var height="750";
	var url = opUrl;
	
	//교육용 독자 미수 출력 팝업 사이즈 조정
	if(eduMisuUrl == opUrl) {	width="350";	height="170"; }
 ...
 * 탑메뉴 이동
 */
function fn_menuMove(prntId, url, nm, subId, popYn) {
	var fm = document.getElementById("masterFm");

	if("Y" == popYn) { //팝업여부 확인
		fn_popReaderEdit(url);
		return false;
	} else {
		fm.target="_self";
		fm.action=url;
...
<body>
<div class="mainBody">
<form name="masterFm" id="masterFm" method="post">
...
<table style="width: 1200px;  height: 80px; padding: 0; border-collapse:collapse;"> 
	...
	<tr style="background-color: #f5f5f5; border-bottom: 6px solid #f48d2e"> 
		<td style="text-align: left; padding-left: 20px"><a href="#fakeUrl" onclick="fn_main_move();"><img src="/images/l_logo.jpg" border="0" /></a></td>
		<!-- 탑메뉴-->
		<td> 
			<div style="width: 1057px;">
				<div style="text-align: right; height: 30px; font-weight: bold; vertical-align: top; margin-top: 10px;">
					...
					<font class="logf"> <a href="http://shop.living24.co.kr/" target="_blank">소모품구매</a></font> <font size="1px;" color="d5d5d5">&nbsp;|&nbsp;</font>
					<!-- ><font class="logf"> <a href="/front/searchJikuk.jsp" target="_blank">지국찾기</a></font> <font size="1px;" color="d5d5d5"></font>  -->
					<font class="logf"> <a href="/zipcode/searchAddr.do" target="_blank">지국찾기</a></font> <font size="1px;" color="d5d5d5">&nbsp;|&nbsp;</font>
					<font class="logf"> <a href="#fakeUrl" onclick="fn_main_move();"> HOME</a></font> <font size="1px;" color="d5d5d5">&nbsp;|&nbsp;</font> 
					<font class="logf"><a href="<%= ISiteConstant.URI_LOGOUT %>">LOGOUT</a></font>&nbsp;&nbsp;&nbsp;
				</div>
				<div style="padding-left:20px; height: 20px;">
					<div style="width: 820px; float: left; padding-bottom: 5px;"> 
						<c:forEach var="list" items="<%=menuList%>" varStatus="i">
							<c:if test="${list.PARENT_MENU_ID ==0}">
								<a href="#fakeUrl" style="text-decoration: none;" onclick="fn_menuMove('${list.MENU_ID}', '${list.MENU_URL}', '${list.MENU_NAME }' , '${list.SUB_MENU_ID }', '${list.POPUP_YN }');">
									<c:choose>
					...
					<div style="width: 210px;  height: 20px; float: left; text-align: right; margin-top: 10px; letter-spacing: -1px"><font class="bname"><c:out value="<%= loginid %>" /> 님</font> 환영합니다.&nbsp;&nbsp;&nbsp;</div>
				</div> 
			</div> 
		</td>
	</tr>
	...
	<tr>
		<td style="vertical-align: top; background-color: #F46F2E; border-top:1px solid #de5613; ">
			<c:set var="topMenuId" value="<%=topMenuId%>"/> 
			<c:choose>
				<c:when test="${topMenuId eq null || empty topMenuId}">
					<!-- 자동이체건수 -->
					<div class="box_m1" style="border-left:1px solid #de5613; border-right:1px solid #de5613;">
						...
						<div style="background-color: #fff">
							...
							<div style="margin-left: 5px;">
								...
								<div id="divGiroInfo_load" style="text-align: center; padding: 16px 0;"><img src="/images/circle_loading2.gif" alt="" style=""/></div> 
							</div> 
						</div>
						<!-- //지로입금 -->
						<!-- 자동이체 -->
						<div style="background-color: #fff">
							...
							<div style="padding: 10px 0; margin-left: 5px;">
								...
								<div id="divResultBill_load" style="text-align: center; padding: 12px 0;"><img src="/images/circle_loading2.gif" alt="" style=""/></div> 
								...
								<div id="divResultBillStu_load" style="text-align: center; padding: 11px 0;"><img src="/images/circle_loading2.gif" alt="" style=""/></div>
							</div>
						</div>
						<!-- //자동이체 -->
					</div>
					...
					<div>
						...
						<div style="text-align: center; background-color: #F46F2E; height: 128px; border-left: 1px solid #de5613; border-right: 1px solid #de5613">
							<table  style="width: 138px; height: 126px; background-color: #fff; border: 0">
							    ...
							    <tr><td style="word-break:break-all;" align="left"><c:import url="/community/bbs/mainNoticeList.do"/></td></tr>
					...
					<c:forEach var="list" items="<%=menuList%>" varStatus="i">
						<c:if test="${list.PARENT_MENU_ID eq prntId && list.MENU_DEPTH eq 2}">			
							<div <c:if test="${subId eq list.MENU_ID }">class="box_m_on"</c:if> <c:if test="${subId ne list.MENU_ID }">class="box_m"</c:if>>
								<a href="#fakeUrl" style="text-decoration: none;" onclick="fn_menuMove('${list.PARENT_MENU_ID}', '${list.MENU_URL}', '${prntNm }', '${list.MENU_ID }', '${list.POPUP_YN }');">${list.MENU_NAME }</a>
							</div>
							<div class="line_m01" style="height: 1px;" /></div>
							...
							<c:forEach var="listSub" items="<%=menuList%>" varStatus="state">
								<c:if test="${listSub.PARENT_MENU_ID eq list.MENU_ID  && listSub.MENU_DEPTH eq 3 && subId eq list.MENU_ID}">
									<div class="box_m_sub"><font class="left3Depth"><a href="#fakeUrl" style="text-decoration: none;" onclick="fn_menuMove('${list.PARENT_MENU_ID}', '${listSub.MENU_URL}', '${prntNm }', '${list.MENU_ID }', '${listSub.POPUP_YN }');">${listSub.MENU_NAME }</a></font></div>
								</c:if>
							</c:forEach>
						</c:if>
					</c:forEach>
			...
			<c:if test="${prntNm eq '독 자' }">
				<!-- 알림판 -->
				<div>
					<div style="padding-top: 32px"><img src="/images/bt_left_alrim.gif" style="vertical-align:middle; border:0" alt=""></div>
					<div style="text-align: center; background-color: #F46F2E; height: 128px; border-left: 1px solid #de5613; border-right: 1px solid #de5613">
						<table  style="width: 138px; height: 126px; background-color: #fff; border: 0">
						    <tr><td align="left" valign="top"><img src="/images/alrim_box1.gif" style="vertical-align: middle;" alt=""></td></tr>
						    <tr><td style="word-break:break-all;" align="left"><c:import url="/community/bbs/mainNoticeList.do"/></td></tr>
		...
		<td style="width: 1057px; vertical-align: top; height: 628px;">
			<div style="padding: 10px 0 15px 10px;  border-top: 1px solid #de5613;" ><decorator:body><page:param name="topMenuId">${topMenuId }</page:param></decorator:body></div>
		</td>
		<!-- main 끝-->
	</tr>
</table>
