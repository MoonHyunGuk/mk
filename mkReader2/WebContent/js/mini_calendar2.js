 ...
 * Update : 2003.03.18
 * Version : 0.3
 * History : 2003.03.18 : SelectBox 자동숨김 기능 추가
 */

var target;                                                                    // 호출한 Object의 저장
var stime;
document.write("<div id=minical oncontextmenu='return false' ondragstart='return false' onselectstart='return false'  style=\"background:buttonface; margin:5px; padding:5px;margin-top:2px;border-top:1px solid buttonshadow;border-left: 1px solid buttonshadow;border-right: 1px solid buttonshadow;border-bottom:1px solid buttonshadow;width:160px;display:none;position: absolute; z-index:100\"></div>");
var auto_hidden_use = "Y"; // 자동숨김옵션 여부
var auto_all = "N"; // 자동으로 모두 숨기면 'Y' 배열의 정의된 것만 숨기려면 'N'

function Calendar(obj_str) {
	var obj = eval(obj_str);
	//해지일자가 등록되어 있을시 해지일자 수정 불가

/*
	//ABC용수금일자입력 컨트롤
	if(obj.name.indexOf("nowSnDt") > -1){
		var tem = obj.name.split("SnDt");
		if(Number(tem[1]) < 12){
			if($(tem[0] +"SgbbCd"+tem[1]).value != '044' ){
				if( $(tem[0]+"ClDt"+tem[1]).value < $(tem[0]+"Year12").value+"20"){
					return;					
				}
			}	
		}
	}
	if(obj.name.indexOf("preSnDt") > -1){
		var tem = obj.name.split("SnDt");
		if($(tem[0] +"SgbbCd"+tem[1]).value != '044'){
			if( $(tem[0]+"ClDt"+tem[1]).value < $("nowYear12").value+"20"){
				return;					
			}
		}
	}
...
//		auto_hidden("visible", ex_data, auto_hidden_use);
//	}else{
//		auto_hidden("hidden", ex_data, auto_hidden_use);
//	}
	

    minical.style.pixelTop    = y+5;
    minical.style.pixelLeft    = x-50-100;
...
function CalendarEX(obj_str, ex_data) {
	var obj = eval(obj_str);
    var now = obj.value.split("-");
    var x, y;
...
function doClick() {                                                            // 날자를 선택하였을 경우
	...
	auto_hidden("visible", "ALL_ELEMENT", auto_hidden_use);
}
...
/*
주의 parseInt는 원래 10진수형으로 반환하는데
08,09 는 0으로 처리됨
해결방법 : parseInt("08",10) 10진수형인 08로 인식시킴
*/	
    ...
    Cal_HTML = "<TABLE BORDER=0 CELLPADDING=0 CELLSPACING=0 ONMOUSEOVER=doOver(); ONMOUSEOUT=doOut(); STYLE='font-size:8pt;font-family:Tahoma; width:170px'>"
...
//                    항상 날짜만 맞으면 색깔 칠해줌
					if (thirdPrintDay==intThisDay) {
                        Cal_HTML += "background-color:#77bbcc;";
                    }                   
                    switch(intLoopDay) {
                        case 1:                                                    // 일요일이면 빨간 색으로
                            Cal_HTML += "color:red;"
                    ...
                    Cal_HTML += "\">"+thirdPrintDay; // + "("+sYear +"-"+ sMonth +"-"+  sDay +"@" + intThisYear +":"+ NowThisYear +","+ intThisMonth +":"+ NowThisMonth +","+ thirdPrintDay +":"+ intThisDay+")"
...
function auto_hidden_all(bool, use_yn){
		...
		for (i=0; i<frms_len; i++){
			...
			for (j=0; j<ele_len; j++){
				if (eles.elements[j].type == "select-one"){
					eles.elements[j].style.visibility = bool;
				}
			}
		}
	}
...
function auto_hidden(bool, ex_data, use_yn){
	if (auto_all == "Y" || ex_data == "ALL_ELEMENT"){
		auto_hidden_all(bool, use_yn);
	}else{
		var frms = document.forms;
		var frms_len = frms.length;
		if (use_yn == "Y"){
			for (i=0; i<frms_len; i++){
				...
				for (j=0; j<ele_len; j++){
					if (eles.elements[j].type == "select-one"){
						if(ex_data.length >0){
							for (k=0; k < ex_data.length ; k++){					
								if(eles.elements[j].name == ex_data[k]){
									eles.elements[j].style.visibility = bool;
								}
							}
						}
