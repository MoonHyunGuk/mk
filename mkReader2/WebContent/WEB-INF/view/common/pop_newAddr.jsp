...
<script type="text/javascript">
	...
	function setAddr(zip , addr , newAddr, bdNm, bdMngNo){
		...
		window.opener.setAddr(zip , addr , newAddr, bdNm, bdMngNo);
		window.close();
...
<body>
<form id="addrSearchForm" name="addrSearchForm" action="/common/common/searchNewAddr.do" method="post" target="_self" enctype="UTF-8">
	...
	<div class="box_Popup" style="overflow: hidden;">
		...
		<div id="container">
		    ...
		    <div class="tab_container">
		        ...
		        <div id="tab1" class="tab_content">
					<input type="text" name="searchValue1" id="searchValue1" class="tabValue" value="<c:if test="${searchType eq 1}">${searchValue}</c:if>" style="ime-mode:active"/>
					<a href="#" onclick="searchAddr();"><img src="/images/bt_search.gif"/></a>
					<p>* 새주소검색은 도로명 기준으로 검색됩니다.   예) 충무로 2</p>
		        </div>
		        <!-- #tab2 -->
		        <div id="tab2" class="tab_content">
					<input type="text" name="searchValue2" id="searchValue2" class="tabValue" value="<c:if test="${searchType eq 2}">${searchValue}</c:if>" style="ime-mode:active"/>
					<a href="#" onclick="searchAddr();"><img src="/images/bt_search.gif"/></a>
					<p>* 건물명으로 검색하세요.   예) 필동 매일경제신문사, 충무로 매일경제신문사</p>
		        </div>
		        <!-- #tab3 -->
		        <div id="tab3" class="tab_content">
					<input type="text" name="searchValue3" id="searchValue3" class="tabValue" value="<c:if test="${searchType eq 3}">${searchValue}</c:if>" style="ime-mode:active"/>
		...
		<div class="clear" id="box_list">
			<c:if test="${empty newAddrList}">
				<div class="box_gray" style="padding: 10px 5px;"> 
					<p><b>조회된 데이터가 없습니다.</b></p><br />
					<p>지번정보가 정확하지 않거나, 도로명 주소데이터가 없는 경우 조회 되지 않을 수 있습니다.</p>
					<p>도로명주소 안내 시스템 을 통해 확인 가능 합니다.</p><br />
			...
			<c:if test="${not empty newAddrList}">
				...
				<table class="tb_list_a_5 clear">
					...
					<tr class="mover" onclick="setAddr('${list.zip_cd}', '${list.beopjeong_text}', '${list.road_text}', '${list.sigungu_building_name}', '${list.building_code}')">
						<td style="text-align:center;">
							${list.zip_cd}
						</td>
						<td style="text-align:left;">${list.road_text}</td>
						<td style="text-align:left;">${list.beopjeong_text}</td>
					</tr>
					</c:forEach>
				</table>
