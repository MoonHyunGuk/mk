...
<div class="box_Popup">
	<!-- title -->
	<div class="pop_title_box"><c:if test="${boseq == '999999'}">학생독자정보</c:if><c:if test="${boseq != '999999'}">일반독자정보</c:if></div>
	<!-- //title -->
	<c:choose>
		...
		<c:otherwise>
		<form name="frmParent" method="post" action="view_db.do" onSubmit="return go();">
		<input type="hidden" name="readno" value="<c:out value='${readno}' />">
			...
			<table class="tb_search" style="width: 550px">
				...
				<tr>
				 	<td width="25%"  bgcolor="f9f9f9" class="box_p">전화번호</td>
					<td >
				...
				<tr>
				 	<th>상세주소</th>
					<td>
						<c:out value="${result.DLVADRS1}" />&nbsp;<c:out value="${result.DLVADRS2}" /> 
					</td>
				</tr>	
				<tr>
				 	<th>구독부수</th>
			...
			<table class="tb_list_a_5" style="width: 550px">
				...
				<c:choose>
					<c:when test="${empty callList}">
						<tr><td colspan="2" align="center">통화 기록이 없습니다.</td></tr>
					</c:when>
					<c:otherwise>
			...
			<div style="width: 550px; text-align: right; padding-top: 10px;">
		  		<c:choose>
					...
					<c:otherwise>
						<!--a href="javascript:document.frmParent.submit();"><img src="/images/bt_save.gif" border="0" align="absmiddle"></a-->
					</c:otherwise>
				</c:choose>
            	<a href="javascript:window.close();"><img src="/images/bt_back.gif" border="0" align="absmiddle"></a>
			</div>
		</form>
		</c:otherwise>
	</c:choose>
