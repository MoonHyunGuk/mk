...
<script type="text/javascript">
	...
	function moveTo(type, pageNo) {
		...
		frm.action = "/billing/zadmin/cmsget/index21.do";
		frm.submit();
	}
	...
	function fn_popErrorView(numId, filename) {
		var width="960";
		var height="400";
		var url = "err21.do?numid="+numId+"&filename="+filename;
...
<div><span class="subTitle">이체청구(일반)</span></div>
<form name="forms1" method="post" action="process21b.do" onsubmit="return goforms();">
<!-- search conditions --> 
<table class="tb_search" style="width: 1020px">
	...
	<tr>
		<th>신청 접수일</th>
		<td colspan="7">
			<input type="text" maxlength="6" name="rdate" style="width: 100px; vertical-align: middle;" onkeydown="return onlyNumber();">&nbsp;예) 050815 <-2005년 8월 15일&nbsp; &nbsp; &nbsp; &nbsp; 
			...
			<c:otherwise>
				<c:forEach items="${resultList}" var="list" varStatus="status">
	                ...
	                <tr class="mover_color">
	                    <td><a href="view21.do?numid=${list.NUMID}&fname=${filename}"><c:out value="${list.NUMID}" /></a></td>
	                    <td><c:out value="${list.LOGDATE}" /></td>
	                    <td><a href="view21.do?numid=${list.NUMID}&fname=${filename}"><c:out value="${filename}" /></a></td>
	                    <td>
	                        <c:choose>
						...
						<td><a href="process21b_excel.do?numid=${list.NUMID}&fname=${filename}">다운</a></td>
					</tr>
				</c:forEach>
			</c:otherwise>
		</c:choose>
...
<!-- //list -->
<form name="searchForm" action="index21.do" method="get">
