...
<script type="text/javascript">
	function fn_goforms() {
		var fm = document.getElemnetById("forms1");
		var cmsfile = document.getElemnetById("cmsfile");
		
		if (cmsfile.value == null && cmsfile.value == '') {
		...
		fm.target ="_self";
		fm.action = "/billing/zadmin/cmsget/input_db.do";
		fm.submit();
	...
	function moveTo(type, pageNo) {
		...
		frm.action = "/billing/zadmin/cmsget/index.do";
		frm.submit();
	}
</script>
<div><span class="subTitle">청구결과(일반)</span></div>
...
<table class="tb_search" style="width: 1020px">
	...
	<tr>
		<th>CMS file</th>
		<td>
			<input type="file" name="cmsfile" id="cmsfile" style="width:400px; vertical-align: middle;" />	&nbsp; &nbsp; &nbsp; &nbsp; 
...
<div style="width: 1020px; margin: 0 auto; padding-top: 15px;">
	<table class="tb_list_a_5" style="width: 1020px">
		...
		<c:choose>
			...
			<tr>
				<td><a href="view.do?numid=${list.NUMID}&filename=${list.FILENAME}&cmsdate=${list.OUT_DATE}&pageNo=${pageNo}"><c:out value="${list.NUMID}" /></a></td>
				<td><c:out value="${list.RTYPE}" /></td>
				<td><a href="view.do?numid=${list.NUMID}&filename=${list.FILENAME}&cmsdate=${list.OUT_DATE}&pageNo=${pageNo}"><c:out value="${list.OUT_DATE}" /></a></td>
				<td><a href="view.do?numid=${list.NUMID}&filename=${list.FILENAME}&cmsdate=${list.OUT_DATE}&pageNo=${pageNo}"><c:out value="${list.FILENAME}" /></a></td>
				<td><c:out value="${list.INDATE}" /></td>
				<td><c:out value="${list.CHK_ID}" /></td>
				<td><a href="view_excel_list.do?numid=${list.NUMID}&filename=${list.FILENAME}&cmsdate=${list.OUT_DATE}">통계</a></td>
			</tr>
			</c:forEach>
		</c:otherwise>
		</c:choose>
