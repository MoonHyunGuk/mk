...
<script type="text/javascript">
	...
	function moveTo(pageNo) {
		...
		frm.action = "/billing/student/cmsrequest/index_stu.do";
		frm.submit();
	}
</script>
<div><span class="subTitle">신청결과(학생)</span></div>
		...
		<c:choose>
			...
			<c:otherwise>
				<c:forEach items="${resultList}" var="list" varStatus="status">
					<tr>
						<td><a href="view_stu.do?numid=${list.NUMID}&filename=${list.FILENAME}&cmsdate=${list.CMSDATE}&pageNo=${pageNo}"><c:out value="${list.NUMID}" /></a></td>
						<td><c:out value="${list.RTYPE}" /></td>
						<td><c:out value="${list.CMSDATE}" /></td>
						<td><a href="view_stu.do?numid=${list.NUMID}&filename=${list.FILENAME}&cmsdate=${list.CMSDATE}&pageNo=${pageNo}"><c:out value="${list.FILENAME}" /></a></td>
...
<form name="searchForm" action="index_stu.do" method="get">
