...
<!--
	...
	function moveTo(type, pageNo) {
		...
		frm.action = "list_stu.do";
		frm.submit();
	}
//-->
</script>
<div><span class="subTitle">환불내역조회(학생)</span>
</div>
<form name="form1" method="post" action="list_stu.do">
...
<div class="box_white" style="padding: 10px 0; width: 1020px">
   ...
   <input type="text" style="vertical-align: middle; width: 85px"  name="sdate" value='<c:out value="${sdate}" />' readonly="readonly" onclick="Calendar(this)"> ~ 
   <input type="text" style="vertical-align: middle; width: 85px" name="edate" value='<c:out value="${edate}" />' readonly="readonly" onclick="Calendar(this)">&nbsp; 
...
<div class="box_list">
	<table class="tb_list_a_5" style="width: 1020px">
		...
		<c:choose>
			<c:when test="${empty resultList}">
				<tr>
					<td colspan="8"  bgcolor="white" align="center" height=70>찾으시는 내용이 없습니다.</td>
				</tr>
			</c:when>
			<c:otherwise>
				<c:forEach items="${resultList}" var="list" varStatus="status">
					<tr bgcolor="ffffff" align="center">
						...
						<td align="left">
							...
							<c:choose>
								<c:when test="${list.GUBUN eq '학생'}">"popMemberViewStu('${list.READNO}');"</c:when>
	...
	<table class="tb_list_a_5" style="width: 1020px">
		...
		<tr>
			<td><input type="text" id="jikuk_name" name="jikuk_name" size="7"/></td>
			<td><input type="text" id="jikuk_code" name="jikuk_code" size="7"/></td>
			<td><input type="text" id="usernumid" name="usernumid" size="7" onKeyPress="inputNumCom();"/></td>
			<td><input type="text" id="username" name="username" size="7"/></td>
			<td><input type="text" id="jumin" name="jumin" size="3"/></td>
			<td><input type="text" id="bank_name" name="bank_name" size="3"/></td>
			<td><input type="text" id="bank_num" name="bank_num" size="3"/></td>
			<td><input type="text" id="refund_price" name="refund_price" size="7" onKeyPress="inputNumCom();"/></td>
			<td><input type="text" id="refund_date" name="refund_date" size="7" onclick="Calendar(this);" onfocus="Calendar(this);" readonly/></td>
		</tr>
	</table>
