...
<script type="text/javascript">
	...
	function fn_detailView(index) {
		...
		clearField();
		jQuery("#boSeq").val(document.getElementById('tem_boSeq0').innerHTML);
		jQuery("#seq").val(document.getElementById('tem_seq'+index).innerHTML);
		jQuery("#readNo").val(document.getElementById('tem_readNo'+index).innerHTML);
		jQuery("#gnoNm").val(document.getElementById('tem_gno'+index).innerHTML+"구역");
		jQuery("#gno").val(document.getElementById('tem_gno'+index).innerHTML);
		jQuery("#bno").val(document.getElementById('tem_bno'+index).innerHTML);
		jQuery("#sno").val(document.getElementById('tem_sno'+index).innerHTML);
		jQuery("#readNm").val(replaceHtml(document.getElementById('tem_readNm'+index).innerHTML));
		jQuery("#bidt").val(document.getElementById('tem_bidt'+index).innerHTML);
		jQuery("#homeTel1").val(document.getElementById('tem_homeTel1'+index).innerHTML);
		jQuery("#homeTel2").val(document.getElementById('tem_homeTel2'+index).innerHTML); 
		jQuery("#homeTel3").val(document.getElementById('tem_homeTel3'+index).innerHTML);
		jQuery("#mobile1").val(document.getElementById('tem_mobile1'+index).innerHTML);
		jQuery("#mobile2").val(document.getElementById('tem_mobile2'+index).innerHTML);
		jQuery("#mobile3").val(document.getElementById('tem_mobile3'+index).innerHTML);
		jQuery("#eMail").val(document.getElementById('tem_eMail'+index).innerHTML);
		jQuery("#dlvZip").val(document.getElementById('tem_dlvZip'+index).innerHTML);
		jQuery("#dlvAdrs1").val(document.getElementById('tem_dlvAdrs1'+index).innerHTML);
		jQuery("#dlvAdrs2").val(replaceHtml(document.getElementById('tem_dlvAdrs2'+index).innerHTML));
		jQuery("#newaddr").val(replaceHtml(document.getElementById('tem_newaddr'+index).innerHTML));
		...
		jQuery("#rsdTypeCd").val(document.getElementById('tem_rsdTypeCd'+index).innerHTML);
		jQuery("#taskCd").val(document.getElementById('tem_taskCd'+index).innerHTML);
		jQuery("#intFldCd").val(document.getElementById('tem_intFldCd'+index).innerHTML);  
		jQuery("#newsCd").val(document.getElementById('tem_newsCd'+index).innerHTML);
		jQuery("#oldNewsCd").val(document.getElementById('tem_newsCd'+index).innerHTML);
		jQuery("#oldGno").val(document.getElementById('tem_gno'+index).innerHTML);
		...
		if(aplcDt != ''){
			jQuery("#hjDt2").val(aplcDt.substring(0,4) + "-" + aplcDt.substring(4,6) + "-" + aplcDt.substring(6,8));
		}
		if(hjDt != ''){
			jQuery("#hjDt").val(hjDt.substring(0,4) + "-" + hjDt.substring(4,6) + "-" + hjDt.substring(6,8));
		}
		
		jQuery("#readTypeCd").val(document.getElementById('tem_readTypeCd'+index).innerHTML);
		jQuery("#oldReadTypeCd").val(document.getElementById('tem_readTypeCd'+index).innerHTML);
		jQuery("#qty").val(document.getElementById('tem_qty'+index).innerHTML);
		jQuery("#sumQty").val(document.getElementById('tem_sumQty'+index).innerHTML);
		jQuery("#oldQty").val(document.getElementById('tem_qty'+index).innerHTML);
		jQuery("#uPrice").val(document.getElementById('tem_uPrice'+index).innerHTML);
		jQuery("#sumUprice").val(document.getElementById('tem_sumUprice'+index).innerHTML);
		jQuery("#dlvTypeCd").val(document.getElementById('tem_dlvTypeCd'+index).innerHTML);
		jQuery("#spgCd").val(document.getElementById('tem_spgCd'+index).innerHTML);
		jQuery("#dlvPosiCd").val(document.getElementById('tem_dlvPosiCd'+index).innerHTML);
		jQuery("#sgType").val(document.getElementById('tem_sgType'+index).innerHTML);
		jQuery("#oldSgType").val(document.getElementById('tem_sgType'+index).innerHTML);
		jQuery("#bnsBookCd").val(document.getElementById('tem_bnsBookCd'+index).innerHTML);
		jQuery("#orgChgDt").val(document.getElementById('tem_chgDt'+index).innerHTML);
		//$("aptCd").val(document.getElementById('tem_aptCd'+index).innerHTML;
		//$("aptNm").val(document.getElementById('tem_aptNm'+index).innerHTML;
		jQuery("#spRemk").val(replaceHtml(document.getElementById('tem_remk'+index).innerHTML));
		//jQuery("#remk").val("");
	
		jQuery("#desc").val("");
		jQuery("#stSayou").val(document.getElementById('tem_stSayou'+index).innerHTML);
		jQuery("#oldStSayou").val(document.getElementById('tem_OldStSayou'+index).innerHTML);
		jQuery("#sgCycle").val(document.getElementById('tem_sgCycle'+index).innerHTML);
		jQuery("#oldSgCycle").val(document.getElementById('tem_sgCycle'+index).innerHTML);
		jQuery("#term").val(document.getElementById('tem_term'+index).innerHTML+"개월");
		jQuery("#boReadNo").val(document.getElementById('tem_boReadNo'+index).innerHTML);
		document.getElementById('callLog').innerHTML =  "<a href=\"javascript:callLog();\">"+document.getElementById('tem_callLog'+index).innerHTML+' 건' +"</a>"; 
 		if(document.getElementById('tem_stdt'+index).innerHTML != ''){
			...
			jQuery("#oldStdt").val(stdt.substring(0,4) + "-" + stdt.substring(4,6) + "-" + stdt.substring(6,8));
			jQuery("#stdt").val(stdt.substring(0,4) + "-" + stdt.substring(4,6) + "-" + stdt.substring(6,8));	
		}else{
			jQuery("#stdt").val("");
		}
		...
		if(sgBgmm != ''){
			jQuery("#sgBgmm").val(sgBgmm.substring(0,4)+"-"+sgBgmm.substring(4,6));	
		}		
		var hjPathCd = document.getElementById('tem_hjPathCd'+index).innerHTML;
		jQuery("#hjPathCd").val(document.getElementById('tem_hjPathCd'+index).innerHTML);
		jQuery("#hjPsnm option:eq(0)").attr("<option value='"+document.getElementById('tem_hjPsnm'+index).innerHTML+"'>"+document.getElementById('tem_hjPsnm'+index).innerHTML+"</option>");
		jQuery("#hjPsregCd option:eq(0)").attr("<option value='"+document.getElementById('tem_hjPsregCd'+index).innerHTML+"'>"+document.getElementById('tem_hjPsregCd'+index).innerHTML+"</option>");
		...
		//수금정보 조회
		//var url = "/collection/collection/ajaxCollectionList_org.do?seq="+$('seq').value+"&readNo="+$('readNo').value+"&newsCd="+$('newsCd').value+"&boSeq="+$('boSeq').value;
		//sendAjaxRequest(url, "readerListForm", "post", collectionList);
		jQuery.ajax({
			type 		: "POST",
			url 		: "/collection/collection/ajaxCollectionList.do",
			dataType 	: "json",
			data		: "seq="+jQuery("#seq").val()+"&readNo="+jQuery("#readNo").val()+"&newsCd="+jQuery("#newsCd").val()+"&boSeq="+jQuery("#boSeq").val(),
			success:function(data){
				var nowYear = '${nowyymm12}'; //현재 사용월분
				var deadLineDt = nowYear+"20";
				var jsonObjArr = data.collectionList;
				
				//초기화
				for(var j=1 ; j < 13 ; j++){
					if(j < 10){ seq ='0'+j; } else { seq = j; }
					
					jQuery("#preBillAmt"+seq).attr("readonly", false);
					jQuery("#preAmt"+seq).attr("readonly", false);
					jQuery("#preSgbbCd"+seq).empty();
					jQuery("#nowBillAmt"+seq).attr("readonly", false);
					jQuery("#nowAmt"+seq).attr("readonly", false);
					jQuery("#nowSgbbCd"+seq).empty();
					jQuery("#nextBillAmt"+seq).attr("readonly", false);
					jQuery("#nextAmt"+seq).attr("readonly", false);
					jQuery("#nextSgbbCd"+seq).empty();
					jQuery("#preSgbbCd"+seq).append("<option value=''></option>");
					jQuery("#nowSgbbCd"+seq).append("<option value=''></option>");
					jQuery("#nextSgbbCd"+seq).append("<option value=''></option>");

					for(var sg=0; sg<data.sgTypeList.length;sg++) {
						jQuery("#preSgbbCd"+seq).append("<option value='"+data.sgTypeList[sg].CODE+"'>"+data.sgTypeList[sg].CNAME+"</option>");
						jQuery("#nowSgbbCd"+seq).append("<option value='"+data.sgTypeList[sg].CODE+"'>"+data.sgTypeList[sg].CNAME+"</option>");
						jQuery("#nextSgbbCd"+seq).append("<option value='"+data.sgTypeList[sg].CODE+"'>"+data.sgTypeList[sg].CNAME+"</option>");
					}
				}
				//셋팅
				if (jsonObjArr.length > 0) {
					for(var j=1 ; j < 13 ; j++){
						...
						for ( var i = 0; i < jsonObjArr.length-2; i++) {
							if(jQuery("#preYear"+seq).val()  ==  jsonObjArr[i].YYMM ){//데이터 존재
								jQuery("#preSnDt"+seq).val(jsonObjArr[i].SNDT);
								jQuery("#preBillAmt"+seq).val(jsonObjArr[i].BILLAMT);
								jQuery("#preAmt"+seq).val(jsonObjArr[i].AMT);
								jQuery("#preClDt"+seq).val(jsonObjArr[i].CLDT);

								if(jsonObjArr[i].SGBBCD != '044'){
									if( Number(jQuery("#preClDt"+seq).val()) < Number(deadLineDt)){
										jQuery("#preSgbbCd"+seq).empty();
										jQuery("#preSgbbCd"+seq).append("<option value='"+jsonObjArr[i].SGBBCD+"' disabled='disabled'>"+jsonObjArr[i].SGBBNM+"</option>");
									// 처리일이 마감일보다 큰경우 수정가능 (2012.06.14 박윤철)
									}else{
										jQuery("#preSgbbCd"+seq).val(jsonObjArr[i].SGBBCD);
									}
								}else{
									jQuery("#preSgbbCd"+seq).val(jsonObjArr[i].SGBBCD);
								}
							...
							if(jQuery("#nowYear"+seq).val()  ==  jsonObjArr[i].YYMM ){
								jQuery("#nowSnDt"+seq).val(jsonObjArr[i].SNDT);
								jQuery("#nowBillAmt"+seq).val(jsonObjArr[i].BILLAMT);
								jQuery("#nowAmt"+seq).val(jsonObjArr[i].AMT);
								jQuery("#nowClDt"+seq).val(jsonObjArr[i].CLDT);
								
								if( Number(jQuery("#nowYear"+seq).val()) < Number(nowYear)  ){
									if(jsonObjArr[i].SGBBCD != '044'){
										if( jQuery("#nowClDt"+seq).val() < Number(deadLineDt)){
											jQuery("#nowSgbbCd"+seq).empty();
											jQuery("#nowSgbbCd"+seq).append("<option value='"+jsonObjArr[i].SGBBCD+"' disabled='disabled'>"+jsonObjArr[i].SGBBNM+"</option>");
										// 처리일이 마감일보다 큰경우 수정가능 (2012.06.14 박윤철)
										}else{
											jQuery("#nowSgbbCd"+seq).val(jsonObjArr[i].SGBBCD);
										}
									}else{
										jQuery("#nowSgbbCd"+seq).val(jsonObjArr[i].SGBBCD);
									}	
							...
							if(jQuery("#nextYear"+seq).val()  ==  jsonObjArr[i].YYMM ){
								jQuery("#nextSnDt"+seq).val(jsonObjArr[i].SNDT);
								jQuery("#nextBillAmt"+seq).val(jsonObjArr[i].BILLAMT);
								jQuery("#nextAmt"+seq).val(jsonObjArr[i].AMT);
								jQuery("#nextSgbbCd"+seq).val(jsonObjArr[i].SGBBCD);
								jQuery("#nextClDt"+seq).val(jsonObjArr[i].CLDT);
							}
						}
					} 
					for ( var i = 0; i < jsonObjArr.length; i++) {
						if (i == jsonObjArr.length - 2) {
							jQuery("#sumgClam").append(jsonObjArr[i].thisYear);
							jQuery("#thisYearSumgClam").val(jsonObjArr[i].thisYear);
						}
						if (i == jsonObjArr.length - 1) {
							jQuery("#lastYearSumgClam").val(jsonObjArr[i].lastYear);
						}
					}
				}
				//현재월부터 이후는 수정가능
				...
				for(var j=1 ; j < 13 ; j++){
					if(j < 10){ seq ='0'+j; } else { seq = j; }
					if(jQuery("#preSgbbCd"+seq).val() == ''){
						jQuery("#preSnDt"+seq).attr("readonly", true);
						jQuery("#preBillAmt"+seq).attr("readonly", true);
						jQuery("#preAmt"+seq).attr("readonly", true);
						jQuery("#preSgbbCd"+seq).empty();
					}else{
						if(jQuery("#preSgbbCd"+seq).val() != '044'){
							// 처리일이 마감일보다 큰경우 수정가능 (2012.06.14 박윤철)
							if(jQuery("#preClDt"+seq).val() < Number(deadLineDt)){
								jQuery("#preBillAmt"+seq).attr("readonly", true);
								jQuery("#preAmt"+seq).attr("readonly", true);
							}
						}
					...
					if( Number(jQuery("#nowYear"+seq).val()) < Number(nowYear)  ){
						if(jQuery("#nowSgbbCd"+seq).val() == ''){
							jQuery("#nowSnDt"+seq).attr("readonly", true);
							jQuery("#nowBillAmt"+seq).attr("readonly", true);
							jQuery("#nowAmt"+seq).attr("readonly", true);
							jQuery("#nowSgbbCd"+seq).empty();
						}else{
							if(jQuery("#nowSgbbCd"+seq).val() != '044'){
								// 처리일이 마감일보다 큰경우 수정가능 (2012.06.14 박윤철)
								if( jQuery("#nowClDt"+seq).val() < Number(deadLineDt)){
									jQuery("#nowSnDt"+seq).attr("readonly", true);
									jQuery("#nowBillAmt"+seq).attr("readonly", true);
			...
			error    : function(r) { alert("sugm ajax error"); }
		}); //ajax end
		...
		jQuery.ajax({
			...
			dataType 	: "json",
			data		: "readNo="+jQuery("#readNo").val(),
			success:function(data){
				jQuery("#desc").val("");
				var div_head = document.createElement("div");
				var div_note = document.createElement("div");
				if (data.memoList.length > 0) {
					for(var j=0 ; j < data.memoList.length ; j++){
						div_note = document.createElement("div");//font-weight: bold;
						div_head = document.createElement("div");
						div_note.onclick = function() {fn_insert_note(data.memoList[j].MEMO);};
						div_head.style.cssText = "font-weight: bold; padding: 3px 0 0 3px";
						div_head.innerText = "["+data.memoList[j].CREATE_ID+"] "+data.memoList[j].CREATE_DATE;
						div_note.innerText = data.memoList[j].MEMO;
						jQuery("#desc").append(div_head);
						jQuery("#desc").append(div_note);
						jQuery("#remk").val(data.memoList[0].MEMO);
					}
				}else{
					div = document.createElement("div");
					jQuery("#div").text("기록이 없습니다.");
		...
		//메모 더보기 클릭이벤트 설정
		document.getElementById("tagMemoLlist").onclick = function(){fn_memo_view_more(jQuery("#readNo").val());};
	...
	function clearField(chkFlag){
		...
		if(document.getElementById("searchType").value == '8'){
		...
		}else if(document.getElementById("searchType").value == '9'){
		...
		document.getElementById("hjPsnm").value = '';
		document.getElementById("hjPsregCd").value = '';
	...
	function fn_save() {
		...
		for(var j=1 ; j < 13 ; j++){
			...
			if($("preSgbbCd"+seq).value != '' && $("preSgbbCd"+seq).value != '044'){
				...
				if($("preSnDt"+seq).value.replace(/(^\s*)|(\s*$)/gi, "") == ''){
					alert('수금일자를 입력해 주세요');
					$("preSnDt"+seq).focus;
					return;
				}
			...
			if($("nowSgbbCd"+seq).value != '' && $("nowSgbbCd"+seq).value != '044'){
				...
				if($("nowSnDt"+seq).value.replace(/(^\s*)|(\s*$)/gi, "") == ''){
					alert('수금일자를 입력해 주세요');
					$("nowSnDt"+seq).focus;
					return;
				}
			...
			if($("nextSgbbCd"+seq).value != '' && $("nextSgbbCd"+seq).value != '044'){
				...
				if($("nextSnDt"+seq).value.replace(/(^\s*)|(\s*$)/gi, "") == ''){
					alert('수금일자를 입력해 주세요');
					$("nextSnDt"+seq).focus;
					return;
				}
	...
	function controlPrice(){
		jQuery("#uPrice").val(jQuery("#qty").val() * 15000);
	}
	//검색 조건 변경
	...
	/**
	 * 기존 행정주소 보기
	function fn_chk_oldaddr(index) {
		alert(document.getElementById('tem_oldaddr'+index).innerHTML);
	}
	*/
				...
				<table class="tb_list" style="width: 993px;">
					...
					<c:forEach items="${readerList }" var="list" varStatus="i">
						<tr style="display: none;">
							...
							<span id="tem_chgDt${i.index}" name="tem_chgDt${i.index}" style="display:none">${list.ORGCHGDT}</span><!--수정일-->
						...
						<c:when test="${list.BNO eq '999'}">
							<tr onclick="fn_detailView('${i.index}')" style="cursor:pointer; background-color: #f9f9f9; color: #e74985">
						...
						<c:when test="${list.READTYPECD eq '099'}">
							<tr onclick="fn_detailView('${i.index}')" style="cursor:pointer; background-color: #E3E3E3">
								<td>${list.GNO }-${list.BNO }</td>
								<td>${list.READNO }</td>
								<td>${list.NEWSNM }</td>
								<td style="text-align: left;"><c:out value="${list.READNM }"/></td>
						...
						<c:otherwise>
							<tr  class="mover" onclick="fn_detailView('${i.index}')">
								<td>${list.GNO }-${list.BNO }</td>
								<td>${list.READNO }</td>
								<td>${list.NEWSNM }</td>
								<td style="text-align: left;"><c:out value="${list.READNM }"/></td>
