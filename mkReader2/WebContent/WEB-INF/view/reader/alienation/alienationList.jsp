...
<script type="text/javascript">
		...
		<c:choose>
			<c:when test="${loginType eq 'A'}">
				if($("searchText").value  != '' || $("boSeq").value  != '' || $("status").value  != '1'){
					...
					alienationListForm.action = "/reader/alienation/searchAlienationList.do";
					alienationListForm.submit();
				}else{
					$("pageNo").value = seq;
					alienationListForm.target = "_self";
					alienationListForm.action = "/reader/alienation/retrieveAlienationList.do";
					alienationListForm.submit();	
				}
			</c:when>
			<c:otherwise>
	...
	function fn_search(){
		...
		fm.action = "/reader/alienation/searchAlienationList.do";
		fm.submit();
	}
	...
	function fn_excel(){
		alienationListForm.target="_self";
		alienationListForm.action = "/reader/alienation/excelAlienationList.do";
		alienationListForm.submit();
	}
	...
	function closeNews(readNo){
		...
		alienationListForm.target="_self";
		alienationListForm.action = "/reader/alienation/closeNews.do";
		alienationListForm.submit();
	}
	...
	function detailVeiw(boSeq , readNo , newsCd , seq){
		...
		<c:choose>
			<c:when test="${loginType eq 'A'}">
				...
				fm.action = "/reader/alienation/alienationInfo.do";
				fm.submit();
			</c:when>
			<c:otherwise>
				fm.target = "_self";
...
<!-- title -->
<div><span class="subTitle">소외계층독자 리스트</span></div>
<!-- //title -->
<form id="alienationListForm" name="alienationListForm" action="" method="post" ENCTYPE="multipart/form-data">
...
<input type=hidden id="opBoSeq" name="opBoSeq" value="" />
	...
	<div class="box_white" style="padding: 10px 0; margin: 0 auto; width: 1020px;">
		...
		&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		<a href="#fakeurl" onclick="fn_excel();"><img src="/images/bt_savexel.gif" style="vertical-align: middle; border: 0;" alt="엑셀저장"></a>
	</div>
	<!-- //search condition -->
	<c:if test="${loginType eq 'A'}">
