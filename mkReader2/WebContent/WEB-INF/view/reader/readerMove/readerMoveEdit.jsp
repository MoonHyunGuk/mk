...
//		return;
//	}
	jQuery("#editForm").attr("action","/reader/readerMove/executeReaderMoveEdit.do").submit();
}

function deleteReaderMove(){
	if(confirm("정말로 삭제 하시겠습니까?")){
		jQuery("#editForm").attr("action","/reader/readerMove/executeReaderMoveDelete.do").submit();
	}
...
<div style="padding: 10px 0 5px 0;"><b><img src="/images/left_icon.gif" style="vertical-align: middle;" alt=""  /> 전출지 정보</b></div>
	...
	<table class="tb_search" style="width: 720px;">
	    ...
	    <tr>
		    <th>이전부수</th>
			<td>
				<input type="text" size="5" name="qty" id="qty" value="${readerMove.QTY}" style="width:45px; ime-Mode:disabled;" onkeypress="inputNumCom();">부
			</td>
	    </tr>
	    ...
	    <tr>
			...
			<td>
				<select name="status" id="status">
					<option value="1"<c:if test="${readerMove.STATUS == '1'}"> selected="selected"</c:if>>처리중</option>
					<option value="2"<c:if test="${readerMove.STATUS == '2'}"> selected="selected"</c:if>>완료</option>
					<option value="3"<c:if test="${readerMove.STATUS == '3'}"> selected="selected"</c:if>>재이전</option>
					<option value="4"<c:if test="${readerMove.STATUS == '4'}"> selected="selected"</c:if>>취소</option>
				</select>
			</td>
	    </tr>
	</table>
...
<div style="width: 720px; text-align: right; padding-top: 10px; padding-left: 15px;">
  	...
  	<a href="#" onclick="deleteReaderMove();return false;"><img src="/images/bt_delete.gif" style="border: 0; vertical-align: middle; "></a>
</div>