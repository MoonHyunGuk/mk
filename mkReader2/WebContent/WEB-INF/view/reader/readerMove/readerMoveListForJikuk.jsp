...
jQuery(document).ready(function($){
	$("#btnInsert").click(function(){
		$(location).attr("href","/reader/readerMove/readerMoveInsertForJikuk.do"); 
	});
	
	$("#btnList").click(function(){
		$("#frmList").attr("action","/reader/readerMove/readerMoveListForJikuk.do").submit();
	});
...
function editReaderMove(seq){
	jQuery("#seq").val(seq);
	jQuery("#frmEdit").attr("action","/reader/readerMove/readerMoveEditForJikuk.do").submit();
}
function moveTo(type, pageNo) {
	...
	frm.action = "readerMoveListForJikuk.do";
	frm.submit();
...
<div>
	<table class="tb_search" style="width: 1020px;">
		...
		<tr>
			...
			<td colspan="3">
				<select name="move_type">
					<option value="ALL"<c:if test="${move_type == 'ALL'}"> selected="selected"</c:if>>전체</option>
					<option value="OUT"<c:if test="${move_type == 'OUT'}"> selected="selected"</c:if>>전출</option>
					<option value="IN"<c:if test="${move_type == 'IN'}"> selected="selected"</c:if>>전입</option>
				</select>
			</td>
		</tr>
		<tr>
		...
		<tr>
			<td colspan="5" style="text-align:left;color:red;border-right:0px;">
				※ 다부수독자 이전처리 등록은 전월 16일 ~ 해당수금월 15일 까지입니다. (15일이 휴일인 경우 그 다음날까지 입력 가능합니다.)
			</td>
	...
	<table class="tb_list_a" style="width: 1020px">  
		...
		<c:forEach items="${readerMoveList}" var="list"  varStatus="status">
			<tr class="mover_color" style="cursor:pointer;" id="${list.SEQ}" onclick="editReaderMove(this.id);">
				...
				<td><div style="text-align:left;padding-left:10px;height:15px;"><c:if test="${list.TEL2 != null}">${list.TEL1}-${list.TEL2}-${list.TEL3}</c:if></div><div style="text-align: left;padding-left:10px;height:15px;"><c:if test="${list.HANDY2 != null}">${list.HANDY1}-${list.HANDY2}-${list.HANDY3}</c:if></div></td>
				<td <c:if test="${move_type != 'ALL'}">style="text-align: left;padding-left:10px;"</c:if>>
				<c:if test="${move_type == 'ALL'}">
				<div style="height:15px;border-bottom:1px solid gray;text-align:left;padding-left:10px;text-overflow: ellipsis;white-space: nowrap;overflow: hidden;width:380px;">${list.OUT_ADRS1}${list.OUT_ADRS2}</div><div style="height:15px;text-align: left;padding-left:10px;text-overflow: ellipsis;white-space: nowrap;overflow: hidden;width:380px;background-color:mistyrose;">${list.IN_ADRS1}${list.IN_ADRS2}</div>
				</c:if>
				<c:if test="${move_type == 'OUT'}">
					${list.OUT_ADRS1}${list.OUT_ADRS2}
				</c:if>
				<c:if test="${move_type == 'IN'}">
					${list.IN_ADRS1}${list.IN_ADRS2}
				</c:if>
				</td>
				...
				<td>${list.MOVEDT}</td>
				<td>
				<b><c:if test="${list.STATUS == '1'}"><font color="blue">처리중</font></c:if><c:if test="${list.STATUS == '2'}"><font color="green">완료</font></c:if><c:if test="${list.STATUS == '3'}"><font color="orange">재이전</font></c:if><c:if test="${list.STATUS == '4'}"><font color="red">취소</font></c:if></b>
				</td>
			</tr>
		</c:forEach>
	</table>
