...
<script type="text/javascript">
	//매체 종류 전체 선택/해제
	function fn_checkControll(){
		var frm = document.frm;
		//전체선택 1 , 전체해제 2
		alert(frm.controll.checked);
...
<div style="padding-bottom: 15px;"> 
	<span class="subTitle">유가현황</span>
</div>
<!-- search conditions -->
<form id="frm" name="frm" method="post">
<input type="hidden" id="type" name="type" value="bill" />
<div>
	...
	<div style="float: left;">
		<div style="border: 1px solid #e5e5e5; padding: 5px; width: 340px">
			<table class="tb_list_a" style="width: 330px">
				...
				<tr>
					<th>&nbsp;</th>
					<th><input type="checkbox" id="controll" name="controll"  onclick="#fakeUrl" onclick="fn_checkControll();" style="border: 0;"> </th>
					<th>코드</th>
					<th>신문명</th>
				</tr>
			</table>
			...
			<table class="tb_list_a" style="width: 313px">
				...
				<c:forEach var="list" items="${newsCodeList}" varStatus="status">
					<tr>
						<td>${status.count}</td>
						<td> 
							<input type="checkbox" id="newsCd" name="newsCd" value="${list.CODE}" 
								<c:choose>
								<%-- 매일경제 기본으로 체크 --%>
	...
	<table class="tb_list_a" style="width: 1020px;">
		...
		<c:forEach var="list" items="${resultList}" varStatus="status">
			<tr>
				...
				<!-- 정가 -->
				<td style="text-align: right;"><fmt:formatNumber value="${list.SUM_LIST_PRICE + list.READTYPECD013 + list.READTYPECD012 + list.READTYPECD015}"  type="number" /></td>
				<!-- 비정가 -->
			...
			<c:set var="SUM_NET_PRICE" value="${SUM_NET_PRICE + list.SUM_LIST_PRICE + list.READTYPECD013 + list.READTYPECD012 + list.READTYPECD015}" />			<!-- 정가 합계 -->
