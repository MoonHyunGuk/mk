...
public class CmsgetController extends MultiActionController implements
	 ...
	 * @throws Exception
	 */
	public ModelAndView index21_stu(HttpServletRequest request,
		...
		// excute query
		List resultList = generalDAO.queryForList("billing.student.cmsget.getEB21LogList", dbparam);
		logger.debug("===== billing.student.cmsget.getEB21LogList");
		
		int t_count = (Integer) generalDAO.queryForObject("billing.student.cmsget.getEB21LogCount");
		logger.debug("===== billing.student.cmsget.getEB21LogCount");
		
		// mav
		ModelAndView mav = new ModelAndView();
		mav.setViewName("billing/student/cmsget/index21_stu");
		mav.addObject("now_menu", ICodeConstant.MENU_CODE_BILLING_STU);
		mav.addObject("show_hidden5", "display");
	 ...
	 * @throws Exception
	 */
	public ModelAndView view21_stu(HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		...
		Map resultMap = (Map) generalDAO.queryForObject("billing.student.cmsget.getEB21LogInfo", dbparam);
		logger.debug("===== billing.student.cmsget.getEB21LogInfo");

		// mav
		ModelAndView mav = new ModelAndView();
		mav.setViewName("billing/student/cmsget/view21_stu");
	 ...
	 * @throws Exception
	 */
	public ModelAndView err21_stu(HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		...
		if ( resultMap != null ) {
			...
			if ( StringUtils.isNotEmpty(err_str)) {
				...
				for ( int i = 0; i < errArr.length; i++ ) {
					...
					dbparam = new HashMap();
					dbparam.put("numid", numid);
					Map userMap = (Map) generalDAO.queryForObject("billing.student.cmsrequest.getUserInfo", dbparam);
					...
					String numidTmp = "00000" + numid;
					numidTmp = numidTmp.substring(numidTmp.length() - 5, numidTmp.length());
					String naknum = "999999" + numidTmp;
					
					Map tmpMap = new HashMap();
					tmpMap.put("NUM", num);
					tmpMap.put("SERIAL", naknum);
		...
		// mav
		ModelAndView mav = new ModelAndView();
		mav.setViewName("billing/student/cmsget/err21_stu");
		mav.addObject("now_menu", MENU_CODE_BILLING_STU);
		mav.addObject("show_hidden5", "display");
	 ...
	 * @throws Exception
	 */
	public ModelAndView process21b_stu(HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		...
		if ( nowDate.compareTo(chedckdate) >= 0 ) {
			...
			mav.addObject("returnURL", "/billing/student/cmsget/index21_stu.do");
			return mav;
		}
		else {
			...
			try {
				...
				//수금테이블에 insert할 readno리스트 -> news테이블, tbl_users_stu 테이블의 sdate조건을 통해 readno를 가지고 온다.
				//(유가년월을 현재월보다 작거나 같은 기준으로 독자번호를 가지고 온다.)
				dbparam = new HashMap();
				dbparam.put("SGTYPE", CODE_SUGM_TYPE_DIRECT_DEBIT);			// 수금방법 - 자동이체
				dbparam.put("READTYPECD", CODE_READER_TYPE_STU_HEAD);		// 독자타입 - 학생본사 
				dbparam.put("STATUS", "EA21");					
				dbparam.put("LEVELS", "3");							
				dbparam.put("NOWMONTH", nowMonth);							
				
				readnoList = generalDAO.getSqlMapClient().queryForList("billing.student.cmsget.getReadnoList", dbparam);
				logger.debug("===== billing.student.cmsget.getReadnoList");
				
				for(int i=0;i<readnoList.size();i++){
					Map readnoMap = (Map)readnoList.get(i);
					String readno = (String)readnoMap.get("READNO");
					
					dbparam = new HashMap();
					dbparam.put("READNO",readno);
					//학생본사 마감과 같은 작업을 해준다(수금테이블에 등록)
					String msg = (String)generalDAO.getSqlMapClient().queryForObject("billing.student.cmsget.funcMonthDeadlineEb21Stu" , dbparam);
					logger.debug("===== billing.student.cmsget.funcMonthDeadlineEb21Stu");
					logger.debug( msg );
				...
				dbparam.put("SGTYPE", CODE_SUGM_TYPE_DIRECT_DEBIT);			// 수금방법 - 자동이체
				dbparam.put("READTYPECD", CODE_READER_TYPE_STU_HEAD);		// 독자타입 - 학생본사 
				dbparam.put("SGBBCD", CODE_SUGM_GUBUN_NOT_COMPLETE);		// 수금구분 - 미수
				dbparam.put("NEWSCD", MK_NEWSPAPER_CODE);					// 신문코드 - 매일경제 신문
				dbparam.put("SGGBCD", CODE_SUGM_TYPE_DIRECT_DEBIT);			// 수금구분 - 자동이체
				//dbparam.put("SDATE", sdate);								// 출금일선택
				dbparam.put("chedckdate", chedckdate.substring(0, chedckdate.length()-2));			// 수금월 (수정 박윤철 12.03.28)
				
				resultList = generalDAO.getSqlMapClient().queryForList("billing.student.cmsget.getSugmList", dbparam);
				logger.debug("===== billing.student.cmsget.getSugmList");
				...
				if ( resultList != null && resultList.size() > 0 ) {
					...
					while ( iter.hasNext() ) {
						...
						/*
							...
							generalDAO.getSqlMapClient().update("billing.student.cmsget.updateSugmToVisit", dbparam);
							logger.debug("===== billing.student.cmsget.updateSugmToVisit");
...
//							}
							...
							generalDAO.getSqlMapClient().insert("billing.student.cmsrequest.insertEALog", dbparam);
							logger.debug("===== billing.student.cmsrequest.insertEALog");
				...
				if ( trailer_str.length() != 150 ) {
					errorch = 1;
					error_str = error_str + "\nTailer 레코드 길이 오류 : " + trailer_str.length() + " bytes";
				}
				...
				if ( StringUtils.isNotEmpty(text) ) {
					...
					try {
						FileUtil.saveTxtFile(
								PATH_UPLOAD_RELATIVE_CMS_STU_EB21 + "/" + year.substring(0, 2) + rdate.substring(0, 2), 
				...
				if ( serialno == 0 ) {
					...
					generalDAO.getSqlMapClient().insert("billing.student.cmsget.insertEB21Log", dbparam);
					logger.debug("===== billing.student.cmsget.insertEB21Log");
					...
					mav.setViewName("billing/student/cmsget/process21b_stu");
					mav.addObject("now_menu", ICodeConstant.MENU_CODE_BILLING_STU);
	 ...
	 * @throws Exception
	 */
	public ModelAndView process21b_stu_excel(HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		...
		// excute query
		String cmsdate = (String)generalDAO.queryForObject("billing.student.cmsget.getLastRequestDate", dbparam);
		logger.debug("===== billing.student.cmsget.getLastRequestDate");
		...
		// excute query
		List resultList = generalDAO.queryForList("billing.student.cmsget.getEALogList", dbparam);
		logger.debug("===== billing.student.cmsget.getEALogList");

		
		// mav
		ModelAndView mav = new ModelAndView();
		mav.setViewName("billing/student/cmsget/process21b_stu_excel");
		mav.addObject("now_menu", ICodeConstant.MENU_CODE_BILLING_STU);
		mav.addObject("show_hidden5", "display");
	 ...
	 * @throws Exception
	 */
	public ModelAndView process21c_stu(HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		...
		Map resultMap = (Map) generalDAO.queryForObject("billing.student.cmsget.getEB21LogFirstRow", dbparam);
		logger.debug("===== billing.student.cmsget.getEB21LogFirstRow");
		...
		// mav
		ModelAndView mav = new ModelAndView();
		mav.setViewName("billing/student/cmsget/process21c_stu");
		mav.addObject("now_menu", ICodeConstant.MENU_CODE_BILLING_STU);
	 ...
	 * @throws Exception
	 */
	public ModelAndView index_stu(HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		...
		// excute query
		List resultList = generalDAO.queryForList("billing.student.cmsget.getEB22LogList", dbparam);
		
		int t_count = (Integer) generalDAO.queryForObject("billing.student.cmsget.getEB22LogCount");
		
		// mav
		ModelAndView mav = new ModelAndView();
		mav.setViewName("billing/student/cmsget/index_stu");
		mav.addObject("now_menu", ICodeConstant.MENU_CODE_BILLING_STU);
		mav.addObject("show_hidden5", "display");
	...
	public ModelAndView view_stu(HttpServletRequest request,
		...
		dbparam.put("jikuk", jikuk);
		int noErrnum1 = (Integer) generalDAO.queryForObject("billing.student.cmsget.getEALogCount", dbparam);
		logger.debug("===== billing.student.cmsget.getEALogCount");
		...
		dbparam.put("cmstype", cmstype);
		dbparam.put("jikuk", jikuk);
		int Errnum1 = (Integer) generalDAO.queryForObject("billing.student.cmsget.getEALogCount", dbparam);
		logger.debug("===== billing.student.cmsget.getEALogCount");
		
		// summary
		dbparam = new HashMap();
		dbparam.put("numid", numid);
		Map resultMap = (Map) generalDAO.queryForObject("billing.student.cmsget.getCmsdataLog", dbparam);
		logger.debug("===== billing.student.cmsget.getCmsdataLog");
		...
		dbparam = new HashMap();
		dbparam.put("CMSDATE", cmsdate);		
		List jikukList = generalDAO.queryForList("billing.student.cmsget.getAgencyList", dbparam);
		logger.debug("===== billing.student.cmsget.getAgencyList");
		...
		// mav
		ModelAndView mav = new ModelAndView();
		mav.setViewName("billing/student/cmsget/view_stu");
		mav.addObject("now_menu", MENU_CODE_BILLING_STU);
		mav.addObject("show_hidden5", "display");
	 ...
	 * @throws Exception
	 */
	public ModelAndView view_excel_stu_list(HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		...
		int pageNo = param.getInt("pageNo", 1);			// 리스트의 몇번째 페이지인지 읽음
		int numid = param.getInt("numid");
		String fromCmsdate = StringUtil.replace(param.getString("fromCmsdate"), "-", "");
		String toCmsdate = StringUtil.replace(param.getString("toCmsdate"), "-", "");
		String filename = fromCmsdate+"~"+toCmsdate;
		String jikuk = param.getString("jikuk");
		...
		dbparam.put("fromCmsdate", fromCmsdate);
		dbparam.put("toCmsdate", toCmsdate);
		
		List resultList = generalDAO.queryForList("billing.student.cmsget.getEB22StatisticsNormalList", dbparam);
		logger.debug("===== billing.student.cmsget.getEB22StatisticsNormalList");
		...
		dbparam.put("fromCmsdate", fromCmsdate);
		dbparam.put("toCmsdate", toCmsdate);
		List errorList = generalDAO.queryForList("billing.student.cmsget.getEB22StatisticsErrorList", dbparam);
		logger.debug("===== billing.student.cmsget.getEB22StatisticsErrorList");
		...
		// mav
		ModelAndView mav = new ModelAndView();
		mav.setViewName("billing/student/cmsget/view_excel_stu_list");
		mav.addObject("now_menu", MENU_CODE_BILLING_STU);
		mav.addObject("show_hidden5", "display");
		...
		mav.addObject("numid", numid);
		mav.addObject("fromCmsdate", fromCmsdate);
		mav.addObject("toCmsdate", toCmsdate);
		mav.addObject("filename", filename);
		mav.addObject("jikuk", jikuk);
		return mav;
	}
	...
	public ModelAndView input_db_stu(HttpServletRequest request,
		...
		if ( cmsfile.isEmpty()) {	// 파일 첨부가 안되었으면 
			...
			mav.addObject("returnURL", "/billing/student/cmsget/index.do");
			return mav;
		}
		else {	// 파일 첨부가 되었으면
			...
			String strFile = fileUtil.saveUploadFile(
											cmsfile, 
											PATH_PHYSICAL_HOME,
											PATH_UPLOAD_ABSOLUTE_CMS_STU_EB22 + "/" + year
										);
			...
			if ( StringUtils.isEmpty(strFile) ) {
				...
				// excute query
				String filename = (String) generalDAO.queryForObject("billing.student.cmsget.getEB22FileName", dbparam);
				logger.debug("===== billing.student.cmsget.getEB22FileName");
				...
				if ( StringUtils.isEmpty(filename)) {
					...
					if ( "EB22".equals(fmt1) ) {
						...
						String fileName = PATH_UPLOAD_RELATIVE_CMS_STU_EB22 + "/" + year + "/" + strFile;
							...
							while ((count = fis.read(b)) != -1) {
								...
								if ( txtline.length() != 0 ) {
									...
									if ( txt_code.equals(CODE_CMS_LAYOUT_HEADER)) {		// 헤더면
										...
										dbparam = new HashMap();
										dbparam.put("cmsresult", "00000");
										dbparam.put("subsmonth", nowYearStr.substring(0, 2) + txt_date.substring(0, 4));
										dbparam.put("cmsdate", txt_date);
										dbparam.put("cmstype", "EA21");									
		
										// excute query
										generalDAO.getSqlMapClient().update("billing.student.cmsget.updateEALog", dbparam);
										logger.debug("===== billing.student.cmsget.updateEALog");
									}
										...
										// excute query
										generalDAO.getSqlMapClient().insert("billing.student.cmsget.insertCmsdata", dbparam);
										logger.debug("===== billing.student.cmsget.insertCmsdata");
									}
										...
										// excute query
										generalDAO.getSqlMapClient().insert("billing.student.cmsget.insertCmsdataLog", dbparam);
										logger.debug("===== billing.student.cmsget.insertCmsdataLog");
										...
										// excute query
										generalDAO.getSqlMapClient().update("billing.student.cmsget.updateSugm", dbparam);
										logger.debug("===== billing.student.cmsget.updateSugm");
									}
								}
							}
				...
				mav.setViewName("billing/student/cmsget/input_db_stu");
				mav.addObject("now_menu", MENU_CODE_BILLING_STU);
	...
	public ModelAndView stat_stu(HttpServletRequest request,
		...
		mav.setViewName("billing/student/cmsget/stat_stu");
		return mav;
	}
	...
	public ModelAndView stat_stu_excel(HttpServletRequest request, HttpServletResponse response) throws Exception
	{
		...
		ModelAndView mav = new ModelAndView();
		mav.addObject("resultList",resultList);
		mav.setViewName("billing/student/cmsget/stat_stu_excel");
		return mav;
	 ...
	 * @throws Exception
	 */
	public ModelAndView err_list_stu(HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		...
		String cmsdate = (String)generalDAO.queryForObject("billing.student.cmsget.getCmsDate", dbparam);
		...
		dbparam = new HashMap();
		dbparam.put("CMSTYPE", "EA21");
		dbparam.put("CMSRESULT1","00000");
		dbparam.put("CMSRESULT2","EEEEE");
		String cmsdateArr[] = null;
		List resultList = null;
		
		if( StringUtils.isNotEmpty(cmsdate) ){
			if( "100122".equals(cmsdate) ){
				cmsdateArr = new String[2];
				cmsdateArr[0] = "100118";
				cmsdateArr[1] = "100122";
				dbparam.put("CMSDATEARR", cmsdateArr);
			}else if( "110318".equals(cmsdate) ){
				cmsdateArr = new String[2];
				cmsdateArr[0] = "110317";
				cmsdateArr[1] = "110318";
				dbparam.put("CMSDATEARR", cmsdateArr);
			}else{
				cmsdateArr = new String[1];
				cmsdateArr[0] = cmsdate;
				dbparam.put("CMSDATEARR", cmsdateArr);
			}
			resultList = generalDAO.queryForList("billing.student.cmsget.getErrListStu", dbparam);
		}
		...
		mav.addObject("resultList",resultList);
		mav.setViewName("billing/student/cmsget/err_list_stu");
	 ...
	 * @throws Exception
	 */
	public ModelAndView err_list_stu_excel(HttpServletRequest request, HttpServletResponse response) throws Exception
	{
		//dbparam
		HashMap dbparam = new HashMap();
		dbparam.put("cmstype", "EA21");
		...
		String fileName = "Student_err_list(" + year + "-" + month +").xls";
		...
		ModelAndView mav = new ModelAndView();
		mav.addObject("resultList",resultList);
		mav.setViewName("billing/student/cmsget/err_list_stu_excel");
